...
def singleline_diff_format(line1, line2, idx):
    ...
    if '\n' in line1+line2 or '\r' in line1+line2:
        return ""
    minlength=min([len(line1),len(line2)])
    if idx > max([minlength,0]) or idx<0:return ""
    string = line1+'\n'+'='*idx+'^\n'+line2 +'\n'
    return string
...
def file_diff_format(filename1, filename2):
    ...
    dif_tuple = multiline_diff(lines1, lines2) # tuple
    if -1 in dif_tuple: return 'No differences\n'
    string = 'Line {}:\n'.format(dif_tuple[0])
    string += singleline_diff_format(lines1[dif_tuple[0]],lines2[dif_tuple[0]],dif_tuple[1])
    return string


#l1 = get_file_lines('file1.txt')



# test 
#print(file_diff_format('file3.txt', 'file2.txt'))    